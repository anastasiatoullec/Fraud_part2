...
def perfomances():
    ...
    """
    dtc_eval = joblib.load('./dtc_eval.pkl')
    return {"Accuracy, %": (dtc_eval['acc']*100).round(2),
           "Precision, %": (dtc_eval['prec']*100).round(2),
           "Recall, %": (dtc_eval['rec']*100).round(2),
           "F1 Score, %": (dtc_eval['f1']*100).round(2),
           "Balanced accuracy, %": (dtc_eval['b']*100).round(2)
        }
...
def predictions3(fraud:FraudDetection):
    ...
    new_case = (pd.DataFrame(features, index = ['0'], columns = ['user_id','signup_day', 'signup_month', 'signup_year', 
        'purchase_day', 'purchase_month', 'purchase_year','purchase_value', 'source','browser','sex','age']))     
    prediction = dtc_model.predict(new_case)
    return {
        "Predicted transaction(1 - fraud, 0 - not fraud)":prediction.tolist()
    }
